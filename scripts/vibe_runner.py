...
def build_prompt(latest_roadmap):
    next_roadmap = f"local/ROADMAP_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    return (
        ...
        f"Your mission as VIBE-RUNNER is twofold: GUI Polish & High-Priority Audio Output Quality Control.\n\n"
        ...
        f"   - Once all tasks in {latest_roadmap} are complete, create the next `{next_roadmap}` preserving all GUI and audio quality mandates."
    )
