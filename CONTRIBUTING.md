...
Thank you for your interest in contributing to **Summoner DAW**! This guide details development practices, real-time safety guardrails, testing workflows, signed commits, and code submission standards.
...
4. **Preserve Documentation:**
   Maintain existing doc comments (`///`) and module level docs (`//!`). Run `cargo test --workspace` to verify that all doctests pass.
...
4. **Dependency & License Audits:** Run `cargo deny check` to verify license compliance and vulnerability safety.
5. **Fuzzing Targets:** If modifying DSP algorithms or audio parsers (WAV/FLAC), test against LibFuzzer targets in `fuzz/fuzz_targets/`:
   ```bash
...
## 4. Signed Commits & Commit Conventions

### Signed Commits Requirement
All commits pushed to `master` or submitted in pull requests MUST be cryptographically signed with GPG or SSH keys.

To configure git commit signing:
```bash
