...
impl OscSaw {
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
    pub fn process_block_simd(&mut self, outputs: &mut [&mut [Sample]], ctx: &ProcessContext) {
        use wide::f32x4;
        let num_samples = outputs[0].len();
...
impl OscWavetable {
    ...
    pub fn with_table2(
        mut self,
        table2: std::sync::Arc<[f32; WAVETABLE_SIZE]>,
        morph: f32,
    ) -> Self {
    ...
    pub fn default_sine() -> std::sync::Arc<[f32; WAVETABLE_SIZE]> {
        ...
        for (i, val) in data.iter_mut().enumerate().take(WAVETABLE_SIZE) {
            *val = (2.0 * std::f32::consts::PI * i as f32 / WAVETABLE_SIZE as f32).sin();
        }
    ...
    pub fn default_saw() -> std::sync::Arc<[f32; WAVETABLE_SIZE]> {
        let mut data = [0.0f32; WAVETABLE_SIZE];
        for (i, val) in data.iter_mut().enumerate().take(WAVETABLE_SIZE) {
            *val = 2.0 * (i as f32 / WAVETABLE_SIZE as f32) - 1.0;
        }
        std::sync::Arc::new(data)
    }
    ...
    pub fn default_square() -> std::sync::Arc<[f32; WAVETABLE_SIZE]> {
        let mut data = [0.0f32; WAVETABLE_SIZE];
        for (i, val) in data.iter_mut().enumerate().take(WAVETABLE_SIZE) {
            *val = if i < WAVETABLE_SIZE / 2 { 1.0 } else { -1.0 };
        }
        std::sync::Arc::new(data)
    }
    ...
    pub fn default_triangle() -> std::sync::Arc<[f32; WAVETABLE_SIZE]> {
        ...
        for (i, val) in data.iter_mut().enumerate().take(WAVETABLE_SIZE) {
            ...
            *val = 2.0 * (2.0 * (phase - (phase + 0.5).floor())).abs() - 1.0;
        }
        std::sync::Arc::new(data)
    }
...
pub fn render_buffer_to_wavetable(samples: &[f32]) -> std::sync::Arc<[f32; WAVETABLE_SIZE]> {
    ...
    for (i, val_ref) in table.iter_mut().enumerate().take(WAVETABLE_SIZE) {
    ...
    for val_ref in table.iter_mut().take(WAVETABLE_SIZE) {
        *val_ref /= max_abs;
    }
...
mod tests {
    ...
    fn test_simd_scalar_agreement_osc_saw() {
        ...
        let mut slices_simd: Vec<&mut [f32]> =
            outputs_simd.iter_mut().map(|v| v.as_mut_slice()).collect();
        ...
        for i in 0..1024 {
            ...
            assert!(
                diff < 1e-3,
                "Mismatch at index {}: SIMD {} vs Scalar {} (diff {})",
                i,
                outputs_simd[0][i],
                outputs_scalar[0][i],
                diff
    ...
    fn test_osc_wavetable_morph() {
        ...
        let mut osc0 =
            OscWavetable::new(440.0, saw_table.clone()).with_table2(square_table.clone(), 0.0);
        let mut osc1 =
            OscWavetable::new(440.0, saw_table.clone()).with_table2(square_table.clone(), 1.0);

        let sample0 = osc0.process_sample(44100);
        let sample1 = osc1.process_sample(44100);
        assert!(
            (sample0 - sample1).abs() > 0.5,
            "Morphing should produce different outputs"
        );
    }
    ...
    fn test_step_1261_simd_poly_wavetable_oscillator() {
        ...
        assert!(
            energy > 1.0,
            "Polyphonic SIMD oscillator block output energy should be non-zero"
        );
        ...
        assert_eq!(
            ...
            0,
            "All voices should decay and become inactive after release"
        );
    }
}
